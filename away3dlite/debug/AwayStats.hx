 ...
 *   <li>Float of polygons in scene</li>
 *   <li>Float of polygons last rendered (yellow in graph)</li>
 * </ul> 
 * 
 ...
 * parameters. By setting the meanDataLength to a non-zero value, the Float of recorded
 * frame rate values on which the average is based can be configured. This has a tiny
 * impact on CPU usage, which is the reason why the default Float is zero, denoting that
...
class AwayStats extends Sprite
{
	...
	private static inline var _MAX_HEIGHT : Float = 75;
	private static inline var _MIN_HEIGHT : Float = 41;
	private static inline var _UPPER_Y : Float = -1;
	private static inline var _LOWER_Y : Float = 9;
	
	private static inline var _POLY_COL : UInt = 0xffcc00;
	private static inline var _MEM_COL : UInt = 0xff00cc;
	...
	private function _redrawStats() : Void
	{
		...
		if (_views.length > 0) {
			_poly_tf.text = Std.int(_rfaces) + ' / ' + _tfaces;
	 ...
	 * the total Float of faces in your scene, and the amount of faces that
	 ...
	 * 
	 * @param meanDataLength The Float of frames on which to base the average
	 ...
	 * respectively.
	*/
	public function new(view3d : View3D = null, minimized : Bool = false, transparent : Bool = false, meanDataLength : UInt = 0, enableClickToReset : Bool = true, enableModifyFrameRate : Bool = true)
	{
		super();
	...
	*/
	public static var instance(get_instance, null):AwayStats;
